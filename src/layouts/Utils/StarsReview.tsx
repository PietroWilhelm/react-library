export const StarsReview: React.FC<{ rating: number, size: number }> = (props) => {
    ...
    if (rating !== undefined && rating > 0 && rating <= 5){
        for (let i = 0; i <= 4; i ++){ // Loop through each star
            if (rating -1 >=0){ // Full star
                fullstars = fullstars +1; // Increment full stars
                rating = rating -1; // Decrement rating
            } else if (rating === .5){ // Half star
                halfstars = halfstars + 1; // Increment half stars
                rating = rating -5; // Decrement rating
            } else if (rating ===0) { // Empty star
                emptystars = emptystars + 1; // Increment empty stars
            } else {
                break;
            }
        }
    } else {
        emptystars = 5; // If rating is invalid, show 5 empty stars
    }
